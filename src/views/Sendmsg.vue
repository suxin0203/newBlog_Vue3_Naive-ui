<template>
  <div class="content">
    <div class="messagebtn" @click="showUpdateModal = true">
    ...
    <n-card>
      <!-- <ZhuD3 /> -->
      <MessageWordcloud :msgList="msgdata" />
    </n-card>
    <n-modal v-model:show="showUpdateModal" preset="dialog" title="Dialog">
      ...
      <div>
        <n-input-group>
          <n-input-group-label>{{ addMeassage.name }}:</n-input-group-label>
          <n-input
      ...
      <template #action>
        <div>
          <n-button type="primary" @click="add(addMeassage)"> 确定 </n-button>
        </div>
      </template>
    </n-modal>
  </div>
